...
def test_make_xgboost_udf_predictions(feature_table, float_model_path):
    """XGBoost UDF predictions should match expected values"""
    predict_udf = ls.expr.ml.make_xgboost_udf(float_model_path)
...
def test_make_xgboost_udf_signature(float_model_path):
    """XGBoost UDF should have correct signature with float64 inputs, float32 output"""
    ...
    assert predict_fn.__name__ == "predict_xgboost"
...
def test_make_xgboost_udf__repr(mixed_model_path):
    """XGBoost UDF repr should include model metadata"""
    predict_fn = ls.expr.ml.make_xgboost_udf(mixed_model_path)
