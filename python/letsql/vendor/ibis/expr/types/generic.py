...
from letsql.vendor.ibis.common.grounds import Singleton
from letsql.vendor.ibis.expr.rewrites import rewrite_window_input
from letsql.vendor.ibis.expr.types.core import (
    Expr,
    _binop,
    _FixedTextJupyterMixin,
    _is_null_literal,
)
from letsql.vendor.ibis.util import deprecated, promote_list, warn_deprecated
...
class Value(Expr):
    ...
    def isin(self, values: Value | Sequence[Value]) -> ir.BooleanValue:
        ...
        └───────────────────┘
        """
        from letsql.vendor.ibis.expr.types import ArrayValue
...
class Scalar(Value):
    ...
    def as_table(self) -> ir.Table:
        ...
        True
        """
        from letsql.vendor.ibis.expr.types.relations import unwrap_alias
...
class Column(Value, _FixedTextJupyterMixin):
    ...
    def topk(
        ...
        └────────┴───────────┘
        """
        from letsql.vendor.ibis.expr.types.relations import bind
...
def literal(value: Any, type: dt.DataType | str | None = None) -> Scalar:
    ...
    >>> from letsql.vendor.ibis import _, selectors as s
    >>> ibis.options.interactive = True
    >>> t = ibis.examples.penguins.fetch()
